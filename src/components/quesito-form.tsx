...
import { PlusCircle, AtSign, User } from 'lucide-react';

type QuesitoFormProps = {
  onAddQuesito: (name: string, igUsername: string) => void;
};
...
export default function QuesitoForm({ onAddQuesito }: QuesitoFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (name.trim() && igUsername.trim()) {
      onAddQuesito(name.trim(), igUsername.trim());
      setName('');
      setIgUsername('');
    }
  };
